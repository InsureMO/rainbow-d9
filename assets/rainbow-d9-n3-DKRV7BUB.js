...
import { c as createLogger, N as NUtils, V as VUtils, k as MonitorNodeAttributes, l as Reaction, E as ExternalDefIndicator, P as PPUtils } from "./rainbow-d9-n1-D7ZeHzAG.js";
import { O as OptionItemSort, R as REACTION_REFRESH_OPTIONS, c as GlobalEventPrefix } from "./rainbow-d9-n2-C6huKoZ5.js";
import { f as fromMarkdown, g as gfmTableFromMarkdown, a as gfmStrikethroughFromMarkdown, b as gfmFootnoteFromMarkdown, c as gfmTaskListItemFromMarkdown, d as frontmatterFromMarkdown } from "./mdast-i-v0Yyqy.js";
import { g as gfmTable, h as gfmStrikethrough, i as gfmFootnote, j as gfmTaskListItem, k as frontmatter } from "./micromark-lcUsp6uI.js";
const $d9 = window;
...
const buildFunc = (async, body, ...argNames) => {
  if (async) {
    const AsyncFunction$1 = AsyncFunction;
    if (argNames == null || argNames.length === 0) {
      return AsyncFunction$1(body);
    } else {
      return AsyncFunction$1(...argNames, body);
    }
  } else {
    const SyncFunction$1 = SyncFunction;
    if (argNames == null || argNames.length === 0) {
      return SyncFunction$1(body);
    } else {
      return SyncFunction$1(...argNames, body);
    }
  }
};
...
const tryBoolAndNumOnAttrValueWithPrefix = (value) => {
  value = (value || "").trim();
  if (VUtils.isBlank(value)) {
    return value;
  }
...
class AbstractMonitorBuild {
  ...
  buildHandleDelegators(monitors) {
...
const createDefaultMonitorHandlerDetective = (options) => {
  ...
  return (options2) => {
    ...
    const handle = buildFunc(true, redressedSnippet, "root", "model", "value", "pathToRoot", "propertyPath", "absolutePath", "changedOn", "from", "to");
...
class ValidatorBuild extends AbstractMonitorBuild {
  ...
  doCombine(monitors, watches, attributes) {
    const createHandle = (delegators) => {
      const func = async (options) => {
        return await monitors.reduce(async (result, { $handle }, index2) => {
          ...
          if ($handle != null) {
            if ($handle instanceof ExternalDefIndicator) {
              return await func.$indicators[index2][0](options) ?? result;
            } else {
    ...
    const createHandle = (delegators) => {
      const func = async (options) => {
        ...
        const results = await Promise.all(monitors.filter(({ $watch }) => {
          ...
          return should;
        }).map(async ({ $handle }, index2) => {
          let ret;
          if ($handle instanceof ExternalDefIndicator) {
            ret = await func.$indicators[index2][0](options);
          } else {
...
class DisablementBuild extends AbstractMonitorBuild {
  ...
  doCombine(monitors, watches, attributes) {
    const createHandle = (delegators2) => {
      const func = async (options) => {
        return await monitors.reduce(async (result, { $handle }, index2) => {
          const ret = await result;
          if (ret) {
          ...
          if ($handle instanceof ExternalDefIndicator) {
            return await func.$indicators[index2][0](options) ?? false;
          } else {
            return await $handle(options) ?? false;
          }
        }, Promise.resolve(false));
      };
      func.$indicators = delegators2;
      return func;
    };
    const createDefaultHandle = (delegators2) => {
      const func = async (options) => {
        return await monitors.reduce(async (result, { $default }, index2) => {
          try {
            const ret = await result;
            if (ret) {
              return ret;
            ...
            if ($default instanceof ExternalDefIndicator) {
              return await func.$indicators[index2][1](options) ?? false;
            } else {
    ...
    const createHandle = (delegators2) => {
      const func = async (options) => {
        return await monitors.reduce(async (result, { $handle }, index2) => {
          ...
          if ($handle instanceof ExternalDefIndicator) {
            return await func.$indicators[index2][0](options) ?? true;
          } else {
      ...
      const func = async (options) => {
        return await monitors.reduce(async (result, { $default }, index2) => {
          try {
            ...
            if ($default instanceof ExternalDefIndicator) {
              return await func.$indicators[index2][1](options) ?? true;
            } else {
...
const InputMaskBuild = createSyncSnippetBuild("mask", ["types"], true);
...
const NumberInputFormatBuild = createSyncSnippetBuild("format", []);
...
const N2DropdownFilterChangedBuild = createSyncSnippetBuild("filterChanged", ["filter", "options"]);
...
const N2CaptionValueToLabelBuild = {
  ...
  build: (value, list) => {
    try {
      const mightBeFunc = createSnippetBuild("valueToLabel", (parsed) => {
      ...
      if (typeof mightBeFunc === "function") {
        return { labelOnValue: true, valueToLabel: mightBeFunc };
      } else {
        return mightBeFunc;
      }
    } catch (e) {
      ...
      if (VUtils.isBlank(attributeName)) {
        ...
        const parsed = (pair.children[0].children ?? []).filter(SemanticUtils.isAttributePairListItem).reduce((attrs, { attributeName: attributeName2, attributeValue: attributeValue2, children }) => {
          ...
          if (name === "label") {
            ...
            attrs.sortKey = attributeValue2.trim();
          }
          return attrs;
        }, {});
        if (VUtils.isNotBlank(parsed.width)) {
          ...
          return { ...parsed, width: N2TranslatorConstants.tableColumnWidth, index: index2 };
        }
      } else {
...
const N2TableSortBuild = createSyncSnippetBuild("sort", ["by", "options"]);
class N2TableTranslator extends SpecificArrayWidgetTranslator {
  ...
  postWork(def, supplementary) {
    ...
    const { $nodes } = defs;
    defs.rowOperators = (_b = ($nodes ?? []).find((node) => {
      return node.$wt === N2WidgetType.TABLE_ROW_OPERATORS || node.$wt.startsWith(`${N2WidgetType.TABLE_ROW_OPERATORS}.`);
    })) == null ? void 0 : _b.$nodes;
    defs.pageable = ($nodes ?? []).find((node) => {
      return node.$wt === N2WidgetType.PAGINATION || node.$wt.startsWith(`${N2WidgetType.PAGINATION}.`);
    });
    defs.$nodes = ($nodes ?? []).filter((node) => {
      return node.$wt !== N2WidgetType.TABLE_ROW_OPERATORS && !node.$wt.startsWith(`${N2WidgetType.TABLE_ROW_OPERATORS}.`) && node.$wt !== N2WidgetType.PAGINATION && !node.$wt.startsWith(`${N2WidgetType.PAGINATION}.`);
    });
    return defs;
  }
}
