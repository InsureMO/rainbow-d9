...
const defaultVisibility: MonitorHandlerDetective = (options: MonitorHandlerDetectOptions): MonitorHandler => {
	...
	if (!handleSnippet.startsWith('return ') && !handleSnippet.startsWith('throw ')) {
		handleSnippet = `return ${handleSnippet}`;
	}
	const handle = new Function('model', 'value', handleSnippet);
	return {
		$watch: on,
		$handle: (options): NodeAttributeValue => {
			const ret = handle(options.model, options.value);
